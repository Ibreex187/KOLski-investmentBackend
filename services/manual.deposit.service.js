...
function formatDepositRecord(tx) {
...
async function createManualDepositRequest(userId, payload = {}) {
...
async function listUserManualDeposits(userId, options = {}) {
  ...
  return {
    items: items.map(formatDepositRecord),
    total,
    page: paging.page,
...
async function getUserManualDepositById(userId, depositId) {
  ...
  return formatDepositRecord(tx);
}

async function listAdminManualDeposits(options = {}) {
  const paging = normalizePaging(options);
  const { filter: createdAtFilter, error: dateError } = buildCreatedAtFilter({
    startDate: options.startDate,
    endDate: options.endDate,
...
module.exports = {
  ...
  listUserManualDeposits,
  getUserManualDepositById,
