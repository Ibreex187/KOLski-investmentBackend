...
function requireManualDepositWritesEnabled(res) {
  const rollout = buildManualDepositRolloutStatus();
  if (rollout.enabled) {
    return null;
  }
...
async function createManualDeposit(req, res) {
  try {
    ...
    if (isTestDbUnavailable()) {
      return success(res, {
        deposit_id: '507f1f77bcf86cd799439012',
        status: 'pending',
    ...
    const result = await manualDepositService.createManualDepositRequest(req.user._id, payload);
    return success(res, {
      ...result,
      message: 'Deposit submitted for manual review',
    }, 201);
...
async function getMyManualDeposits(req, res) {
  try {
    if (isTestDbUnavailable()) {
      return success(res, {
        items: [],
    ...
    const data = await manualDepositService.listUserManualDeposits(req.user._id, {
      page: req.query.page,
...
async function getMyManualDepositById(req, res) {
  try {
    if (isTestDbUnavailable()) {
      return success(res, {
    ...
    const data = await manualDepositService.getUserManualDepositById(req.user._id, req.params.id);
    return success(res, data);
  } catch (err) {
    return error(res, err.message, resolveErrorStatus(err));
  }
...
async function listAdminManualDeposits(req, res) {
  try {
    if (isTestDbUnavailable()) {
      return success(res, {
        items: [],
    ...
    const data = await manualDepositService.listAdminManualDeposits({
      page: req.query.page,
      limit: req.query.limit,
      status: safeStatusFilter(req.query.status),
      startDate: req.query.startDate,
...
async function approveManualDeposit(req, res) {
  try {
    ...
    const data = await manualDepositService.approveManualDeposit({
...
async function rejectManualDeposit(req, res) {
  try {
    ...
    const data = await manualDepositService.rejectManualDeposit({
      depositId: req.params.id,
      adminUserId: req.user._id,
      reason: req.body.reason,
      adminNote: req.body.admin_note,
