function parseDate(value) {
  if (!value) return null;
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return null;
  return date;
}

function buildManualWithdrawalRolloutStatus(now = new Date(), env = process.env) {
  const isProduction = env.NODE_ENV === 'production';
  const rawFlag = env.MANUAL_WITHDRAWALS_ENABLED;
  const baseEnabled = rawFlag === undefined ? !isProduction : rawFlag === 'true';

  const startDate = parseDate(env.MANUAL_WITHDRAWALS_WINDOW_START);
  const endDate = parseDate(env.MANUAL_WITHDRAWALS_WINDOW_END);

  if (!baseEnabled) {
    return {
      enabled: false,
      reason: 'MANUAL_WITHDRAWALS_ENABLED is disabled',
      window_start: startDate ? startDate.toISOString() : null,
      window_end: endDate ? endDate.toISOString() : null,
    };
  }

  if (startDate && now < startDate) {
    return {
      enabled: false,
      reason: 'Manual withdrawals are not active yet for the rollout window',
      window_start: startDate.toISOString(),
      window_end: endDate ? endDate.toISOString() : null,
    };
  }

  if (endDate && now > endDate) {
    return {
      enabled: false,
      reason: 'Manual withdrawals are outside the rollout window',
      window_start: startDate ? startDate.toISOString() : null,
      window_end: endDate.toISOString(),
    };
  }

  return {
    enabled: true,
    reason: null,
    window_start: startDate ? startDate.toISOString() : null,
    window_end: endDate ? endDate.toISOString() : null,
  };
}

module.exports = {
  buildManualWithdrawalRolloutStatus,
};
